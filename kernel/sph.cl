...
)
{
	...
	for (int i = 0; i < N; i++) {
		...
		if (length(n[i]) > 0) {
			if (i!=id)
				nabla_n +=  dot(n[i]/length(n[i]), gradW(r, h)) / body_rho[i];
		}
	}	
...
constant int confIndex[] = {0,1,2,
							...
							// additional cases to cope with ambiguity (reduce number of holes in surface)
							0,1,3,1,3,5,1,5,4,1,2,4,
							0,1,5,1,3,5,0,5,6,0,4,6,0,2,4,
							1,4,6,2,3,5,2,5,7,0,2,7,0,7,8							
							};
...
{
	int t = (int)(W((float4)(0.0125,0,0,0), 0.075) * m * 10000000000);
	int Cnt;
